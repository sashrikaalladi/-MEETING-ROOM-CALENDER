...
import { dateRangeLabel } from 'CalendarWebPartStrings';
import { createLessThan, createThis } from 'typescript';
import { event } from 'jquery';
import { ISPList } from '@pnp/spfx-controls-react';
...
export class Event extends React.Component<IEventProps, IEventState> {
  ...
  //private locationDropdownOption: IDropdownOption[] = [];
  ...
  public constructor(props) {
    ...
    this.state = {
      ...
      startSelectedHour: { key: '10', text: '00' },
      startSelectedMin: { key: '00', text: '00' },
      endSelectedHour: { key: '19', text: '00' },
    ...
    this.onGetErrorMessagelocation = this.onGetErrorMessagelocation.bind(this);
    //this.onGetErrorMessageHour = this.onGetErrorMessageHour.bind(this);
  ...
  public async OnClickCheckCondition() {
    ...
    try {
      const eventsListData: any = await this.spService.getEvents(escape(this.props.siteUrl), escape(this.props.list), new Date(), new Date());
      console.log(eventsListData);
      console.log("this is eventslistdata");
      const doMap = eventsListData.map((x) => {
        return (x.EventDate);
    ...
    eventData.EventDate = actualStart;
    console.log(startTime, startDateTime, start);
    ...
    // if((eventData.EventDate>=endDateTime)&&(end<=startDateTime)){
    ...
    switch (panelMode) {
      ...
      case IPanelModelEnum.add:
        ...
        break;
      case IPanelModelEnum.view:
        await this.spService.viewEvent(eventData, this.props.siteUrl, this.props.listId);
        break;
      default:
        break;
    }
  ...
  private async onSave() {
    ...
    console.log(eventData.fAllDayEvent);
    //Map above eventsData and check condition
    ...
    const startTime = `${this.state.startSelectedHour.key}:${this.state.startSelectedMin.key}`;
    const startDateTime = `${startDate} ${startTime}`;
    let start = moment(startDateTime, 'YYYY/MM/DD HH:mm').toLocaleString();
    const actualStart = new Date(start);
    eventData.EventDate = actualStart;
    console.log(startTime, startDateTime, start);
    ...
    eventData.EndDate = actualEnd;
    console.log(actualEnd);
    console.log(actualStart);
    //DateVALLIDATION
    try {
      const eventsListData: any = await this.spService.getEvents(escape(this.props.siteUrl), escape(this.props.list), this.props.eventStartDate.value, this.props.eventEndDate.value);
      console.log(eventsListData);
      console.log("this is eventslistdata");
      ...
      const checkDateCondition = eventsListData.filter((x: any) => { return x.location1 === this.state.eventData.location1 && x.EventDate.getDate() === actualStart.getDate() && x.EventDate.getMonth() === actualStart.getMonth() && x.EventDate.getFullYear() === actualStart.getFullYear() })
      if (checkDateCondition.length > 0) {
        console.log(actualStart.getTime());
        const checkHourValidation = checkDateCondition.filter((x: any) => { return ((((actualStart.getHours() * 60 + actualStart.getMinutes()) > (x.EventDate.getHours() * 60 + x.EventDate.getMinutes()) && (actualStart.getHours() * 60 + actualStart.getMinutes()) < (x.EndDate.getHours() * 60 + x.EndDate.getMinutes())) || ((actualEnd.getHours() * 60 + actualEnd.getMinutes()) > (x.EventDate.getHours() * 60 + x.EventDate.getMinutes()) && ((actualEnd.getHours() * 60 + actualEnd.getMinutes()) < (x.EndDate.getHours() * 60 + x.EndDate.getMinutes()))))) })
        const checkouterHourValidation = checkDateCondition.filter((x: any) => { return ((((actualStart.getHours() * 60 + actualStart.getMinutes()) > (x.EndDate.getHours() * 60 + x.EndDate.getMinutes())) || ((actualStart.getHours() * 60 + actualStart.getMinutes()) < (x.EventDate.getHours() * 60 + x.EventDate.getMinutes()) && ((actualEnd.getHours() * 60 + actualEnd.getMinutes()) < (x.EndDate.getHours() * 60 + x.EndDate.getMinutes()))))) })
        if (checkHourValidation.length > 0) {
          let listEventId = checkHourValidation[0].ID
          let formId = eventData.ID;
          if (checkHourValidation.length === 1 && formId === listEventId) {
            this.setState({ isSaving: true });
          }
          else {
            console.log("you are selecting in between the event");
            this.setState({ hasError: true, errorMessage: "This slot is in between another meeting slot,Please select another slot", isSaving: false });
            console.log(this.state.isSaving);
          }
        }

        else if (checkouterHourValidation.length == 0) {
          console.log("you are selecting in between the event");
          this.setState({ hasError: true, errorMessage: "This slot is in between another meeting slot,Please select another slot", isSaving: false });
          console.log(this.state.isSaving);
          ...
          const sameEventCheck = checkDateCondition.filter((x: any) => { return ((actualStart.getHours() * 60 + actualStart.getMinutes()) === (x.EventDate.getHours() * 60 + x.EventDate.getMinutes())) && ((actualEnd.getHours() * 60 + actualEnd.getMinutes()) === (x.EndDate.getHours() * 60 + x.EndDate.getMinutes())) })

          if (sameEventCheck.length > 0) {
            let listEventId = sameEventCheck[0].ID
            let formId = eventData.ID;
            if (panelMode === 2 && listEventId === formId) {
              console.log("sameevent");
              this.setState({ isSaving: true });
            }
            else {
              this.setState({ hasError: true, errorMessage: "This slot is already booked", isSaving: false });
            }
          }
          else {
            this.setState({ isSaving: true });
  ...
  private async renderEventData(eventId?: number) {
    ...
    // Edit Mode ?
    this.locationDropdownOption = await this.spService.getChoiceFieldOptions(this.props.siteUrl, this.props.listId, 'location1');
  ...
  private onLocationChanged(ev: React.FormEvent<HTMLDivElement>, item: IDropdownOption): void {

    this.setState({ eventData: { ...this.state.eventData, location1: item.text } });
    console.log(this.state.eventData.location1);
  }
  ...
  * @memberof Event
  */
  private onGetErrorMessagelocation(value: string): string {

    let returnMessage: string = '';

    if (value.length === 0) {
      returnMessage = strings.EventLocationErrorMessage;
    } else {
      this.setState({ disableButton: false, errorMessage: '' });
    }
    return returnMessage;
        ...
        }
        {
          ...
          <>
            ...
            <PrimaryButton
              ...
              onClick={this.onSave}
              //onGetErrorMessage={this.onGetErrorMessageTime}
              style={{ marginBottom: '15px', marginRight: '8px', float: 'right' }}>
              ...
              <div>
                ...
                <div style={{ marginTop: 10 }} >
                  <TextField
                    ...
                    enabled={this.state.userPermissions.hasPermissionAdd || this.state.userPermissions.hasPermissionEdit ? false : true}
                  />
                ...
                <div>
                  <Dropdown
                    ...
                    selectedKey={this.state.eventData && this.state.eventData.location1 ? this.state.eventData.location1 : ''}
                    //defaultSelectedKey={this.state.eventData.location1 ? this.state.eventData.location1 : ''}
                    onChange={this.onLocationChanged}
                    errorMessage={this.onGetErrorMessagelocation}
