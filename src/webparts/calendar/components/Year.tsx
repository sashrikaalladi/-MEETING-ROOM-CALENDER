...
function CalendarDate(props) {
  ...
  if (dateToRender.month() < dateOfMonth.month()) {
    return (
      <button disabled={true} className={css("date", ["prevMonth"])}>
        {dateToRender.date()}
      </button>
    );
  }
  ...
  if (dateToRender.month() > dateOfMonth.month()) {
    return (
      <button disabled={true} className={css("date",["nextMonth"])}>
        {dateToRender.date()}
      </button>
    );
  }
  ...
  return (
    <button
      className={`${css("date", "inMonth")} ${today}`}
