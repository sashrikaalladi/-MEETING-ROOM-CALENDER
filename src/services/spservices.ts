...
export default class spservices {
  ...
  public async getEvent(siteUrl: string, listId: string, eventId: number): Promise<IEventData> {
    ...
    try {
      ...
      returnEvent = {
        ...
        location1: event.location1? event.location1:'',
  ...
  public async updateEvent(updateEvent: IEventData, siteUrl: string, listId: string) {
    ...
    try {
      ...
      let newItem: any = {
        ...
        EventDate: new Date(eventDate),
        EndDate: new Date(endDate),
        Location: updateEvent.location1,
  ...
  public async viewEvent(viewEvent: IEventData, siteUrl: string, listId: string) {
    let results = null;
    try {
      // delete all recursive extentions before update recurrence event
      if (viewEvent.EventType.toString() == "1") await this.deleteRecurrenceExceptions(viewEvent, siteUrl, listId);

      const web = new Web(siteUrl);
      const eventDate = await this.getUtcTime(viewEvent.EventDate);
      const endDate = await this.getUtcTime(viewEvent.EndDate);
      ...
      let newItem: any = {
        ...
        //Geolocation: updateEvent.geolocation,
        ParticipantsPickerId: { results: viewEvent.attendes },
        EventDate: new Date(eventDate),
        EndDate: new Date(endDate),
        Location: viewEvent.location1,
        fAllDayEvent: viewEvent.fAllDayEvent,
        fRecurrence: viewEvent.fRecurrence,
        Category: viewEvent.Category,
        RecurrenceData: viewEvent.RecurrenceData ? await this.deCodeHtmlEntities(viewEvent.RecurrenceData) : "",
        EventType: viewEvent.EventType,
      };

      if (viewEvent.UID) {
        newItem.UID = viewEvent.UID;
      }
      if (viewEvent.MasterSeriesItemID) {
        newItem.MasterSeriesItemID = viewEvent.MasterSeriesItemID;
      }

      results = newItem;
      //await web.lists.getById(listId).items.getById(viewEvent.Id).update(newItem);
    } 
    catch (error) {
      return Promise.reject(error);
    }
  ...
  public async getUserById(userId: number, siteUrl: string): Promise<SiteUser> {
    ...
    try {
      const web = new Web(siteUrl);
    const actualUser= await web.currentUser.get();
  ...
  public async getCurrentUser(siteUrl: string): Promise<[]> {
   // let results: SiteUser = null;
let actualUser:[];

    try {
      const web = new Web(siteUrl);
     actualUser= await web.currentUser.get();
  ...
  public async getUserPermissions(siteUrl: string, listId: string): Promise<IUserPermissions> {
    ...
    try {
      console.log(siteUrl,listId);
      const web = new Web(siteUrl);

      const userEffectivePermissions = await web.lists.getById(listId).effectiveBasePermissions.get();
      const perms = await sp.web.lists.getById(listId).getCurrentUserEffectivePermissions();
      const isAddPerms=sp.web.hasPermissions(perms, PermissionKind.AddListItems);
      console.log(perms);
      console.log(isAddPerms);
      if(sp.web.hasPermissions(perms, PermissionKind.AddListItems))
      {
        console.log("i have permission");
      }
      else{
        console.log("u dont have permission");
      }
      console.log(userEffectivePermissions);
  ...
  //To load Events by RESOURCEROOM

  public async getEventsByResourceRoom(siteUrl: string, listId: string, eventStartDate: Date, eventEndDate: Date,resourceRoom:string): Promise<IEventData[]> {
   console.log("hi this resourceevents",resourceRoom);
...
//               console.log(event.location1,resourceRoom);
// if(event.location1===resourceRoom)
// {
//   console.log("gotit")
      ...
      events = parseEvt.parseEvents(events, null, null);
       ...
       const eventByResource=events.filter((item)=>{return item.location1===resourceRoom});
       console.log(eventByResource);
      // Return Data
      return eventByResource;
